...
시스템 간 인터페이스 전문에서 헤더와 본문(body)을 포함한 JSON 예시를 작성해보겠습니다. 여기서는 주문 정보를 전달하는 예시를 들어 설명하겠습니다.
...
```json
{
  "header": {
    ...
    "destinationSystem": "InventorySystem",
    "transactionId": "789e0123-e45b-67d8-c901-234567890abc",
    "correlationId": "345e6789-e12b-34c5-f678-901234567890",
    "version": "1.0",
...
- **header**: 앞서 설명한 바와 같이, 메시지의 메타데이터 정보를 포함합니다.
- **body**: 실제 데이터 전송의 본문으로, 주문에 대한 상세 정보를 담고 있습니다.

#### Body 필드 설명
- **orderId**: 주문의 고유 식별자입니다.
- **orderDate**: 주문 날짜입니다.
- **customer**: 고객 정보입니다.
  - **customerId**: 고객의 고유 식별자입니다.
  - **name**: 고객의 이름입니다.
  - **email**: 고객의 이메일 주소입니다.
  - **phone**: 고객의 연락처 번호입니다.
- **items**: 주문된 제품 목록입니다. 각 항목은 제품 ID, 설명, 수량, 단가, 그리고 통화를 포함합니다.
- **totalAmount**: 주문의 총액입니다.
- **currency**: 금액의 통화입니다.
- **paymentMethod**: 결제 수단입니다.
- **shippingAddress**: 배송 주소 정보입니다.

이와 같은 JSON 전문은 시스템 간의 데이터 교환을 명확하고 체계적으로 수행하는 데 유용합니다. 필요한 경우 각 프로젝트나 서비스의 요구 사항에 맞춰 구조를 확장하거나 수정할 수 있습니다.

**Q1**: JSON 형식의 데이터 전송에서 데이터를 유효성 검사하는 방법은 무엇인가요?

**Q2**: 인터페이스 설계 시 헤더와 본문 간의 관계를 어떻게 관리할 수 있을까요?

**Q3**: 시스템 간의 데이터 교환에서 발생할 수 있는 오류를 어떻게 처리하고 해결할 수 있을까요?

<img width="2502" alt="image" src="https://github.com/user-attachments/assets/d096e856-b63e-46a2-a4fd-955ef6cc1d75">
